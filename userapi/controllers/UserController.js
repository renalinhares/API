...
var secret = "icjdivjdssvjsvvkjsdkcivjssklmsçkcklçlçvjmd";
...
class UserController{
    ...
    async changePassword(req,res){
        ...
        var isTokenValidate = await PasswordToken.validate(token);

        if(isTokenValidate.status){
            await User.changePassword(password,isTokenValidate.token.user_id,isTokenValidate.token.token);
            res.status(200);
